...
{
   Assert(mTablesBuilt == false, "Cannot build tables twice!");
   mTablesBuilt = true;
   ...
   while (currWraps != 1) {
      ...
      Assert(index1 != -1 && index2 != -1 && index1 != index2, "hrph");
   ...
   Assert(currWraps == 1, "wrong wraps?");
   Assert(pWrap[0].pNode != NULL && pWrap[0].pLeaf == NULL, "Wrong wrap type!");
...
{
   if (rWrap.pLeaf != NULL) {
      Assert(rWrap.pNode == NULL, "um, never.");
...
bool HuffmanStringProcessor::writeHuffBuffer(BitStream* pStream, const char* out_pBuffer, U32 maxLen)
{
   ...
   AssertV(len <= 255, ("String \"%s\" TOO long for writeString", out_pBuffer));
   if (len > maxLen)
      len = maxLen;
