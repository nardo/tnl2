...
#define TORQUE_TEST_RESOURCEMANAGER
...
#if defined(TORQUE_TEST_RESOURCEMANAGER)

int main(int argc, const char **argv)
{
   ResourceManager::create();
   const char *path = ".";
   gResourceManager->setModPaths(1, &path);

   ResourceManager::destroy();
   U32 lastGTC = GetTickCount();
   U32 lastTime = U32(Time::getCurrent().getTotalMilliseconds());
   for(;;)
   {
      U32 nextGTC = GetTickCount();
      U32 nextTime = U32(Time::getCurrent().getTotalMilliseconds());
      if(nextGTC != lastGTC || nextTime != lastTime)
      {
         lastGTC = nextGTC;
         lastTime = nextTime;
         logprintf("TickCount: %d  Time: %d", nextGTC, nextTime);
      }
      //Torque::sleep(0);
   }
}
...
public:
   ...
   TNL_DECLARE_RPC(rpcVectorTest, (IPAddress theIP, ByteBufferPtr buf, Vector<IPAddress> v0, Vector<String> v1, Vector<F32> v2));
...
TNL_IMPLEMENT_RPC(RPCTestConnection, rpcVectorTest,
    (IPAddress theIP, ByteBufferPtr buf, Vector<IPAddress> v0, Vector<String> v1, Vector<F32> v2),
    (theIP, buf, v0, v1, v2),
      NetClassGroupGameMask, RPCGuaranteedOrdered, RPCDirAny, 0)
{
...
int main(int argc, const char **argv)
{
   TorqueLogEnable(LogConnectionProtocol, true);
...
int main(int argc, const char **argv)
{
   TorqueLogEnable(LogNetInterface, true);
