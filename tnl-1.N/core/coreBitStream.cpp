...
#include "core/coreHuffmanStringProcessor.h"
...
{
	Assert(classType < NetClassTypeCount, "Out of range class type.");
	Assert(classId < ClassRep::getNetClassCount(classGroup, classType), "Out of range class id.");
	writeInt(classId, ClassRep::getNetClassBitSize(classGroup, classType));
}

U32 BitStream::readClassId(U32 classType, U32 classGroup)
{
   Assert(classType < NetClassTypeCount, "Out of range class type.");
   U32 ret = readInt(ClassRep::getNetClassBitSize(classGroup, classType));
   if(ret >= ClassRep::getNetClassCount(classGroup, classType))
      return 0xFFFFFFFF;
   return ret;
}
