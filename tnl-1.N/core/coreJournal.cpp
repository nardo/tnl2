...
Journal::Journal()
{
   Assert(mJournal == NULL, "Cannot construct more than one Journal instance.");
   mJournal = this;
}
...
void Journal::callEntry(const char *funcName, Functor<BitStream> *theCall)
{
   if(mCurrentMode == Playback)
      return;

   Assert(mInsideEntrypoint == false, "Journal entries cannot be reentrant!");
...
void Journal::beginBlock(U32 blockId, bool writeBlock)
{
   ...
   {
#ifdef TNL_ENABLE_BIG_JOURNALS
      JOURNAL_WRITE( (U16(0x1234 ^ blockId)) );
#endif
   }
   else
   {
...
void Journal::endBlock(U32 blockId, bool writeBlock)
{
   ...
   {
#ifdef TNL_ENABLE_BIG_JOURNALS
      JOURNAL_WRITE( (U16(0x5678 ^ blockId)) );
#endif
      syncWriteStream();
   }
   else
