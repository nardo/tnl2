...
#ifndef _CORE_JOURNAL_H_
#define _CORE_JOURNAL_H_

#include "core/coreMethodDispatch.h"
#include <stdio.h>

#define TORQUE_ENABLE_JOURNALING
//#define TORQUE_ENABLE_BIG_JOURNALS
namespace TNL
{
...
public:
   ...
   void callEntry(const char *funcName, Functor<BitStream> *theCall);
...
#ifdef TORQUE_ENABLE_JOURNALING
#define DECLARE_JOURNAL_ENTRYPOINT(func, args) \
      void func args; \
      void func##_body args

#define IMPLEMENT_JOURNAL_ENTRYPOINT(className, func, args, argNames) \
      struct Journal_##className##_##func##_er : public TNL::JournalEntryRecord { \
      TNL::FunctorDecl<TNL::BitStream, void (className::*) args> mFunctorDecl; \
      Journal_##className##_##func##_er(const char *name) : TNL::JournalEntryRecord(name), mFunctorDecl(&className::func##_body) { mFunctor = &mFunctorDecl; } \
...
public:
   JournalToken(U32 blockType, bool writing)
   {
      ...
      Journal::beginBlock(mBlockType, mWriting);
   }

   ~JournalToken()
   {
      Journal::endBlock(mBlockType, mWriting);
   }
};
...
#define JOURNAL_WRITE_BLOCK(blockType, x) \
{ \
...
#define JOURNAL_READ_BLOCK(blockType, x) \
{ \
...
#define JOURNAL_READ(x) \
   TNL::Journal::getReadStream()->read x

#define JOURNAL_WRITE(x) \
...
#define IMPLEMENT_JOURNAL_ENTRYPOINT(className, func, args) \
   void className::func args
