...
/// ClassRep class instances represent each declared NetClass
/// and are used to construct object instances.
///
/// Core functionality for Torque-registered class manipulation.
///
/// @section ClassRep_intro Introduction (or, Why AbstractClassRep?)
///
/// Torque requires the ability to programatically instantiate classes, by name or by
/// ID numbers. This is used for handling network connections, ghosting, working with
...
/// ClassRep and ClassRepInstance are all about. They allow the registration
...
///       Core::Object::create(). Most of the time, you will only ever need to use this part
...
///
/// @section ClassRep_details ClassRep Internals
///
/// NCR does some preparatory work at runtime before execution is passed to main(), through static
/// initialization. In actual fact, this preparatory work is done by the ClassRepInstasnce
/// template. Let's examine this more closely.
///
/// If we examine Core::Object, we see that two macros must be used in the definition of a
/// properly integrated objects. Let's look at an example:
///
/// @code
///      // This is from inside the class definition...
///      TORQUE_DECLARE_CLASS(TestObject);
///
/// // And this is from outside the class definition...
/// TORQUE_IMPLEMENT_CLASS(TestObject);
/// @endcode
///
...
/// @code
///      // This is from inside the class definition...
///      static ClassRepInstance<TestObject> dynClassRep;
///      static ClassRep* getParentStaticClassRep();
///      static ClassRep* getStaticClassRep();
///      virtual ClassRep* getClassRep() const;
/// @endcode
///
/// @code
/// // And this is from outside the class definition...
/// ClassRep* TestObject::getClassRep() const { return &TestObject::dynClassRep; }
/// ClassRep* TestObject::getStaticClassRep() { return &dynClassRep; }
/// ClassRep* TestObject::getParentStaticClassRep() { return Parent::getStaticClassRep(); }
/// ClassRepInstance<TestObject> TestObject::dynClassRep("TestObject", 0, -1, 0);
/// @endcode
///
/// As you can see, getClassRep(), getStaticClassRep(), and getParentStaticClassRep() are just
/// accessors to allow access to various ClassRepInstance instances. This is where the Parent
...
/// on. Much like Torque's ConsoleConstructor, ClassRepInstances add themselves to a global linked
/// list in their constructor.
///
/// Then, when ClassRep::initialize() is called, we iterate through
...
protected:
   ...
   /// Next declared ClassRep.
   ///
   /// These are stored in a linked list built by the macro constructs.
   ClassRep *mNextClass;

   static ClassRep *mClassLinkList;                                      ///< Head of the linked class list.
   static Vector<ClassRep *> mClassTable[NetClassGroupCount][NetClassTypeCount]; ///< Table of ClassReps for construction by class ID.
...
public:
   ...
   static ClassRep *getClass(U32 classGroup, U32 classType, U32 index)
   ...
   /// Logs the bit usage information of all the ClassReps
   static void logBitUsage();
};

inline U32 ClassRep::getClassId(NetClassGroup classGroup) const
{
   return mClassId[classGroup];
}

inline NetClassType ClassRep::getClassType() const
{
   return mClassType;
}

inline S32 ClassRep::getClassVersion() const
{
   return mClassVersion;
}

inline const char * ClassRep::getClassName() const
{
   return mClassName;
}

inline U32 ClassRep::getClassGroupCRC(NetClassGroup classGroup)
{
...
/// ClassRepInstance - one of these templates is instantiated for each
/// class that is declared via the IMPLEMENT_* macros below.
///
/// There will be an instance for each networkable class.
template <class T>
class ClassRepInstance : public ClassRep
{
public:
   /// Each class registers itself through the constructor of its NetClassInstance
   ClassRepInstance(const char *className, U32 groupMask, NetClassType classType, S32 classVersion)
   {
   ...
   /// Each ClassRepInstance overrides the virtual create() function to construct its object instances.
   Object *create() const
   {
      return new T;
   }
...
///
/// This class is always used with the TORQUE_DECLARE_CLASS and TORQUE_IMPLEMENT_CLASS macros.
///
/// @code
...
///
/// @see ClassRepInstance for gory implementation details.
/// @nosubgrouping
...
public:
   /// Returns the ClassRep associated with this object.
   virtual ClassRep* getClassRep() const;
   ...
   /// These helper functions let you create an instance of a class by name or ID.
   ///
   /// @note Call these instead of ClassRep::create
   /// @{

   /// static function to create an instance of a named class
   static Object* create(const char* className) { return ClassRep::create(className); }
   ...
   static Object* create( const NetClassGroup groupId,
                          ...
                          const U32 classId)
   {
      return ClassRep::create(groupId, typeId, classId);
   }
...
// the TORQUE_DECLARE_CLASS(className) macro needs to be placed
// in the class declaraction for each network aware class.
//
// Different IMPLEMENT_CLASS_* macros can be used depending on
// which NetClassType the class belongs to.

/// The TORQUE_DECLARE_CLASS macro should be called within the declaration of any network class
#define TORQUE_DECLARE_CLASS(className) \
   static TNL::ClassRepInstance<className> dynClassRep;      \
   virtual TNL::ClassRep* getClassRep() const

/// The TORQUE_IMPLEMENT_CLASS macro should be used for classes that will be auto-constructed
/// by name only.
#define TORQUE_IMPLEMENT_CLASS(className) \
   TNL::ClassRep* className::getClassRep() const { return &className::dynClassRep; } \
   TNL::ClassRepInstance<className> className::dynClassRep(#className, 0, TNL::NetClassTypeNone, 0)
