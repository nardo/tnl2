...
{
   JOURNAL_READ_BLOCK(Socket::Socket,
         JOURNAL_READ( (&mPlatformSocket) );
      return;
   )
   init();
   ...
   {
      TorqueLogMessageV(LogUDP, ("Attempted to create a socket bound to an invalid transport."));
   }
   if(mPlatformSocket != INVALID_SOCKET)
   {
      S32 error = 0;

      // Zero means no bind (Using GGCNatTunnel...)
      if(bindAddress.port != 0)
      {
         ...
         TorqueLogMessageV(LogUDP, ("%s socket created - bound to address: %s", socketType, boundAddress.toString()));
      }
      ...
      if(!error)
      {
         TorqueLogMessageV(LogUDP, ("%s socket receive buffer size set to %d.", socketType, recvBufferSize));
         error = setsockopt(mPlatformSocket, SOL_SOCKET, SO_SNDBUF, (char *) &sendBufferSize, sizeof(sendBufferSize));
      }
      else
         TorqueLogMessageV(LogUDP, ("%s socket error: unable to set the receive buffer size on socket.", socketType));

      if(!error)
      {
         TorqueLogMessageV(LogUDP, ("%s socket send buffer size set to %d.", socketType, recvBufferSize));
         ...
         TorqueLogMessageV(LogUDP, ("%s socket error: unable to set the send buffer size on socket.", socketType));
      ...
      {
         TorqueLogMessageV(LogUDP, ("%s socket error: unable to set broadcast mode on socket.", socketType));
      }

      if(!error)
      {
         TorqueLogMessageV(LogUDP, ("%s socket non-blocking IO set.  Socket initialized.", socketType));
      }
      else
      {
         TorqueLogMessageV(LogUDP, ("Error prevented successful initialization of %s socket.", socketType));
         closesocket(mPlatformSocket);
         mPlatformSocket = INVALID_SOCKET;
      }
   }
   JOURNAL_WRITE_BLOCK(Socket::Socket,
      JOURNAL_WRITE( (mPlatformSocket) );
   )
}
...
{
   JOURNAL_READ_BLOCK(Socket::sendto,
      return NoError;
   )

   JOURNAL_WRITE_BLOCK(Socket::sendto, ;
   )
...
{
   JOURNAL_READ_BLOCK(Socket::recvfrom,
      bool wouldBlock;
      JOURNAL_READ( (&wouldBlock) );
      if(wouldBlock)
         return WouldBlock;
      
      JOURNAL_READ( (&address->transport) );
      JOURNAL_READ( (&address->port) );
      JOURNAL_READ( (&address->netNum[0]) );
      JOURNAL_READ( (&address->netNum[1]) );
      JOURNAL_READ( (&address->netNum[2]) );
      JOURNAL_READ( (&address->netNum[3]) );
      JOURNAL_READ( (outSize) );
      JOURNAL_READ( (*outSize, buffer) );
      return NoError;
   )
   ...
   {
      JOURNAL_WRITE_BLOCK(Socket::recvfrom,
         JOURNAL_WRITE ( (true) );
      )
      return WouldBlock;
   }
   ...
   *outSize = bytesRead;
   ...
   JOURNAL_WRITE_BLOCK(Socket::recvfrom,
      JOURNAL_WRITE( (false) );
      JOURNAL_WRITE( (address->transport) );
      JOURNAL_WRITE( (address->port) );
      JOURNAL_WRITE( (address->netNum[0]) );
      JOURNAL_WRITE( (address->netNum[1]) );
      JOURNAL_WRITE( (address->netNum[2]) );
      JOURNAL_WRITE( (address->netNum[3]) );
      JOURNAL_WRITE( (*outSize) );
      JOURNAL_WRITE( (*outSize, buffer) );
   )
   return NoError;
}
