...
{
   ClassRep::initialize(); // initialize the net class reps, if they haven't been initialized already.
   ...
   mCurrentTime = TNL::getRealMilliseconds();
}
...
{
   mCurrentTime = getRealMilliseconds();
...
{
   TorqueLogMessageV(LogNetInterface, ("Sending Connect Challenge Request to %s", conn->getNetAddress().toString()));
   PacketStream out;
...
{
   TorqueLogMessageV(LogNetInterface, ("Received Connect Challenge Request from %s", addr.toString()));
...
void NetInterface::sendConnectChallengeResponse(const Address &addr, Nonce &clientNonce, bool wantsKeyExchange, bool wantsCertificate)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Sending Challenge Response: %8x", identityToken));
...
void NetInterface::handleConnectChallengeResponse(const Address &address, BitStream *stream)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Received Challenge Response: %8x", theParams.mClientIdentity ));
...
void NetInterface::continuePuzzleSolution(NetConnection *conn)
{
   ...
   if(solved)
   {
      logprintf("Client puzzle solved in %d ms.", getRealMilliseconds() - conn->mConnectLastSendTime);
...
{
   TorqueLogMessageV(LogNetInterface, ("Sending Connect Request"));
   PacketStream out;
   ConnectionParameters &theParams = conn->getConnectionParameters();
...
void NetInterface::handleConnectRequest(const Address &address, BitStream *stream)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Received Connect Request %8x", theParams.mClientIdentity));
...
{
   TorqueLogMessageV(LogNetInterface, ("Sending Connect Accept - connection established."));
...
void NetInterface::handleConnectAccept(const Address &address, BitStream *stream)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Received Connect Accept - connection established."));
}
...
void NetInterface::handleConnectReject(const Address &address, BitStream *stream)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Received Connect Reject - reason %s", reason));
...
void NetInterface::sendPunchPackets(NetConnection *conn)
{
   ...
   for(S32 i = 0; i < theParams.mPossibleAddresses.size(); i++)
   {
      ...
      TorqueLogMessageV(LogNetInterface, ("Sending punch packet (%s, %s) to %s",
...
void NetInterface::handlePunch(const Address &theAddress, BitStream *stream)
{
   ...
   TorqueLogMessageV(LogNetInterface, ("Received punch packet from %s - %s", theAddress.toString(), b.encodeBase64()->getBuffer()));
   ...
   TorqueLogMessageV(LogNetInterface, ("Punch from %s matched nonces - connecting...", theAddress.toString()));
...
{
   TorqueLogMessageV(LogNetInterface, ("Sending Arranged Connect Request"));
   PacketStream out;
...
void NetInterface::handleArrangedConnectRequest(const Address &theAddress, BitStream *stream)
{
   ...
   theParams.mDebugObjectSizes = stream->readFlag();
   stream->read(&connectSequence);
   TorqueLogMessageV(LogNetInterface, ("Received Arranged Connect Request"));
