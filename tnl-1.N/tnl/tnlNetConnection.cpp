...
void NetConnection::writeRawPacket(BitStream *bstream, NetPacketType packetType)
{
   ...
   if(packetType == DataPacket)
   {
      ...
      TorqueLogMessageV(LogNetConnection, ("NetConnection %s: START %s", mNetAddress.toString(), getClassName()) );
      writePacket(bstream, note);
      TorqueLogMessageV(LogNetConnection, ("NetConnection %s: END %s - %d bits", mNetAddress.toString(), getClassName(), bstream->getBitPosition() - start) );
   }
...
void NetConnection::readRawPacket(BitStream *bstream)
{
   ...
   {
      TorqueLogMessageV(LogNetConnection, ("NetConnection %s: RECVDROP - %d", mNetAddress.toString(), getLastSendSequence()));
      return;
   }
   TorqueLogMessageV(LogNetConnection, ("NetConnection %s: RECV- %d bytes", mNetAddress.toString(), bstream->getMaxReadBitPosition() >> 3));
...
void NetConnection::writePacketHeader(BitStream *stream, NetPacketType packetType)
{
   ...
   //if(isNetworkConnection())
   //{
   //   TorqueLogMessageV(LogBlah, ("SND: mLSQ: %08x  pkLS: %08x  pt: %d abc: %d",
   //      mLastSendSeq, mLastSeqRecvd, packetType, ackByteCount));
   //}

   TorqueLogMessageV(LogConnectionProtocol, ("build hdr %d %d", mLastSendSeq, packetType));
}
...
bool NetConnection::readPacketHeader(BitStream *pstream)
{
   ...
   //if(isNetworkConnection())
   //{
   //   TorqueLogMessageV(LogBlah, ("RCV: mHA: %08x  pkHA: %08x  mLSQ: %08x  pkSN: %08x  pkLS: %08x  pkAM: %08x",
   ...
   for(U32 i = 0; i < notifyCount; i++) 
   {
      ...
      TorqueLogMessageV(LogConnectionProtocol, ("Ack %d %d", notifyIndex, packetTransmitSuccess));
...
void NetConnection::sendPingPacket()
{
   ...
   TorqueLogMessageV(LogConnectionProtocol, ("send ping %d", mLastSendSeq));
...
void NetConnection::sendAckPacket()
{
   ...
   TorqueLogMessageV(LogConnectionProtocol, ("send ack %d", mLastSendSeq));
...
{
   TorqueLogMessageV(LogNetConnection, ("NetConnection %s: NOTIFY %d %s", mNetAddress.toString(), sequence, recvd ? "RECVD" : "DROPPED"));
   ...
   if(recvd)
   {
      ...
      if(isAdaptive())
      {
         ...
         if(cwnd < ssthresh)
         {
            // Slow start strategy
            cwnd++;
            TorqueLogMessageV(LogNetConnection, ("PKT SSOK - ssthresh = %f     cwnd=%f", ssthresh, cwnd));
            ...
            TorqueLogMessageV(LogNetConnection, ("PKT   OK - ssthresh = %f     cwnd=%f", ssthresh, cwnd));
...
/*         TorqueLogMessageV(LogNetConnection, ("  * ack=%f   pktDt=%d    time=%f (%d)     seq=%d %d %d %d",
                        ack,
                        ackDelta,
                        deltaT / 1000.0f,
                        deltaT,
...
NetError NetConnection::sendPacket(BitStream *stream)
{
   if(mSimulatedPacketLoss && Random::readF() < mSimulatedPacketLoss)
   {
      TorqueLogMessageV(LogNetConnection, ("NetConnection %s: SENDDROP - %d", mNetAddress.toString(), getLastSendSequence()));
      return NoError;
   }

   TorqueLogMessageV(LogNetConnection, ("NetConnection %s: SEND - %d bytes", mNetAddress.toString(), stream->getBytePosition()));
...
void NetConnection::writeConnectRequest(BitStream *stream)
{
   ...
   stream->write(U32(ClassRep::getClassGroupCRC(getNetClassGroup())));
}
...
bool NetConnection::readConnectRequest(BitStream *stream, const char **errorString)
{
   ...
   if(classGroup == getNetClassGroup() && classCRC == ClassRep::getClassGroupCRC(getNetClassGroup()))
      return true;
