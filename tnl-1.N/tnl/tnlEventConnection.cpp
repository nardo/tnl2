...
EventConnection::~EventConnection()
{
   ...
   while(mNotifyEventList)
   {
      ...
      mEventNoteChunker.freeElem(temp);
   }
...
void EventConnection::writeConnectRequest(BitStream *stream)
{
   ...
   stream->write(ClassRep::getNetClassCount(getNetClassGroup(), NetClassTypeEvent));
}
...
bool EventConnection::readConnectRequest(BitStream *stream, const char **errorString)
{
   ...
   U32 myCount = ClassRep::getNetClassCount(getNetClassGroup(), NetClassTypeEvent);
   if(myCount <= classCount)
      mEventClassCount = myCount;
   else
   {
      mEventClassCount = classCount;
      if(!ClassRep::isVersionBorderCount(getNetClassGroup(), NetClassTypeEvent, mEventClassCount))
         return false;
   }
   mEventClassVersion = ClassRep::getClass(getNetClassGroup(), NetClassTypeEvent, mEventClassCount-1)->getClassVersion();
   ...
   while(walk)
   {
      switch(walk->mEvent->mGuaranteeType)
      {
         case NetEvent::GuaranteedOrdered:
            ...
            TorqueLogMessageV(LogEventConnection, ("EventConnection %s: DroppedGuaranteed - %d", getNetAddressString(), walk->mSeqCount));
         ...
         case NetEvent::Unguaranteed:
            ...
            temp = walk->mNextEvent;
            mEventNoteChunker.freeElem(walk);
            walk = temp;
      }
   }
}
...
void EventConnection::packetReceived(PacketNotify *pnotify)
{
   ...
   while(mNotifyEventList && mNotifyEventList->mSeqCount == mLastAckedEventSeq + 1)
   {
      ...
      TorqueLogMessageV(LogEventConnection, ("EventConnection %s: NotifyDelivered - %d", getNetAddressString(), mNotifyEventList->mSeqCount));
      mNotifyEventList->mEvent->notifyDelivered(this, true);
      mEventNoteChunker.freeElem(mNotifyEventList);
...
void EventConnection::writePacket(BitStream *bstream, PacketNotify *pnotify)
{
   ...
   while(mUnorderedSendEventQueueHead)
   {
      ...
      TorqueLogMessageV(LogEventConnection, ("EventConnection %s: WroteEvent %s - %d bits", getNetAddressString(), ev->mEvent->getDebugName(), bstream->getBitPosition() - start));
...
void EventConnection::readPacket(BitStream *bstream)
{
   ...
   while(true)
   {
      ...
      if(mConnectionParameters.mDebugObjectSizes)
      {
         Assert(endingPosition == bstream->getBitPosition(),
      ...
      note->mEvent = evt;
      note->mSeqCount = seq;
      TorqueLogMessageV(LogEventConnection, ("EventConnection %s: RecvdGuaranteed %d", getNetAddressString(), seq));
   ...
   while(mWaitSeqEvents && mWaitSeqEvents->mSeqCount == mNextRecvEventSeq)
   {
      ...
      TorqueLogMessageV(LogEventConnection, ("EventConnection %s: ProcessGuaranteed %d", getNetAddressString(), temp->mSeqCount));
