...
void GhostConnection::writePacket(BitStream *bstream, PacketNotify *pnotify)
{
   ...
   for(S32 i = mGhostZeroUpdateIndex - 1; i >= 0 && !bstream->isFull(); i--)
   {
      ...
      if(!bstream->writeFlag(walk->flags & GhostInfo::KillGhost))
      {
         ...
         TorqueLogMessageV(LogGhostConnection, ("GhostConnection %s GHOST %d", walk->obj->getClassName(), bstream->getBitPosition() - 16 - startPos));
...
void GhostConnection::readPacket(BitStream *bstream)
{
   ...
   while(bstream->readFlag())
   {
      ...
      else
      {
         ...
         if(mConnectionParameters.mDebugObjectSizes)
         {
            Assert(bstream->getBitPosition() == endPosition,
...
void GhostConnection::activateGhosting()
{
   ...
   mGhostingSequence++;
   TorqueLogMessageV(LogGhostConnection, ("Ghosting activated - %d", mGhostingSequence));
...
{
   TorqueLogMessageV(LogGhostConnection, ("Got GhostingStarting %d", sequence));
...
TNL_IMPLEMENT_RPC(GhostConnection, rpcReadyForNormalGhosts, 
      ...
      NetClassGroupGameMask, RPCGuaranteedOrdered, RPCDirAny, 0)
{
   TorqueLogMessageV(LogGhostConnection, ("Got ready for normal ghosts %d %d", sequence, mGhostingSequence));
   if(!doesGhostFrom())
   {
      setLastError("Invalid packet.");
      return;
