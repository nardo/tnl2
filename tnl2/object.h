...
/// that allow us to send the object over the network. TORQUE_IMPLEMENT_CLASS() doesn't perform these tasks, see
...
protected:
   ...
   BitMask32 mNetFlags;  ///< Flags field describing this object, from NetFlag.
...
public:
   ...
   static void collapseDirtyList()
	{
	   ...
	   {
		  Assert(tempV[i]->mNextDirtyList == NULL && tempV[i]->mPrevDirtyList == NULL && tempV[i]->mDirtyMaskBits == 0, "Error in collapse");
	   }
	}
   ...
   void setMaskBits(U32 orMask)
	{
	   Assert(orMask != 0, "Invalid net mask bits set.");
	   Assert(mDirtyMaskBits == 0 || (mPrevDirtyList != NULL || mNextDirtyList != NULL || mDirtyList == this), "Invalid dirty list state.");
	   if(!mDirtyMaskBits)
	   {
		  Assert(mNextDirtyList == NULL && mPrevDirtyList == NULL, "Object with zero mask already in list.");
		  if(mDirtyList)
		  {
   ...
   void postRPCEvent(NetObjectRPCEvent *theEvent)
	{
	   ...
	   Assert((!isGhost() && theEvent->mRPCDirection == RPCToGhost) ||
...
#define TNL_IMPLEMENT_NETOBJECT(className) \
   TORQUE_IMPLEMENT_NETWORK_CLASS(className, Torque::NetClassTypeObject, Torque::NetClassGroupGameMask, 0)
   ...
   void process(EventConnection *ps)
	{
	   ...
	   mFunctor->dispatch(mDestObject.getPointer());
...
class RPCEV_##className##_##name : public Torque::NetObjectRPCEvent { \
public: \
   Torque::FunctorDecl<Torque::BitStream, void (className::*)args> mFunctorDecl;\
   RPCEV_##className##_##name(Torque::NetObject *theObject = NULL) : Torque::NetObjectRPCEvent(theObject, guaranteeType, eventDirection), mFunctorDecl(&className::name##_remote) { mFunctor = &mFunctorDecl; } \
   TORQUE_DECLARE_CLASS( RPCEV_##className##_##name, Torque::RPCEvent ); \
