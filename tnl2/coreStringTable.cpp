...
#include "core/coreDataChunker.h"
...
/// This is internal to the StringTable namespace.
struct Node
{
...
void validate()
{
   ...
   Assert(nodeCount == mItemCount, "Error!!!");
   U32 freeListCount = 0;
   ...
   while(walk)
   {
      ...
      if(!((walk >> 1) < mNodeListSize))
         Assert((walk >> 1) < mNodeListSize, "Out of range node index!!!");
      freeListCount++;
   }
   Assert(freeListCount + nodeCount == mNodeListSize, "Error!!!!");
   ...
   for(U32 i = 0; i < mNumBuckets; i++)
   {
      ...
      while(walk)
      {
        Assert(walk < mNodeListSize, "Out of range node index!!!");
        Node *node = mNodeList[walk];
        Assert((U32(node) & 1) == 0, "Free list entry in node chain!!!");
        Assert(walk == node->masterIndex, "Master/node index mismatch.");
        walk = node->nextIndex;
      }
   }
}
...
StringTableEntryId insertn(const char* val, S32 len, const bool caseSens)
{
   ...
   *walk = stringNode->masterIndex;
   ...
   Assert(!mNodeListFreeEntry || (mNodeListFreeEntry & 1), "Error in freeList!!");
